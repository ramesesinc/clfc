...
#define MinJRE "1.6"
#define WebJRE "http://dl.filehorse.com/win/browsers-and-plugins/java-runtime-32/Java-Runtime-Environment-1.6.0.45-(32-bit).exe?st=jCyKWxEigOuyJui5yGtO7A&e=1386056774&fn=jre-6u45-windows-i586.exe"
...
begin
  ...
  begin
    ...
    //Log('{#WebJRE}');
    ITD_AddFile('{#WebJRE}', ExpandConstant('{tmp}\jre_1.6-32.exe'));
...
begin
  if CurStep=ssInstall then begin //Lets install those files that were downloaded for us
    Exec(ExpandConstant('{tmp}\jre_1.6-32.exe'), '', '', SW_SHOW, ewWaitUntilTerminated, ResultCode);
  end;
end;